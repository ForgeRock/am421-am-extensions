...
//        addScopesBasedOnPrivileges(validatedScope, request);
    ...
    public UserInfoClaims getUserInfo(
        ...
        //TODO Ch5L1Ex2Task3: Add a new claim called "expires_in" which should contain the token's time to live (TTL) in seconds.
        //TODO Ch5L1Ex2Task3: If the token is not null, calculate the TTL: (token.getExpiryTime() - System.currentTimeMillis()) / 1000
        //TODO Ch5L1Ex2Task3: Put this value to userInfoClaims.getValues() with the key "expires_in".
        if (token != null) {
            List<String> privileges = extractPrivileges(token);
            userInfoClaims.getValues().put("contactlist-privileges", privileges);
            userInfoClaims.getValues().put("expires_in", 100);
